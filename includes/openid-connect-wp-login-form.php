...
/**
 * OpenID_Connect_WP_Login_Form class.
 *
 ...
 * @category  Login
 */
class OpenID_Connect_WP_Login_Form {
	 ...
	 * @param OpenID_Connect_WP_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 */
	public function __construct( OpenID_Connect_WP_Option_Settings $settings, OpenID_Connect_WP_Client_Wrapper $client_wrapper ) {
		$this->settings = $settings;
		$this->client_wrapper = $client_wrapper;
	}

	/**
	 * Create an instance of the OpenID_Connect_WP_Login_Form class.
	 *
	 * @param OpenID_Connect_WP_Option_Settings $settings       A plugin settings object instance.
	 * @param OpenID_Connect_WP_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 *
	 * @return void
	 */
	public static function register( OpenID_Connect_WP_Option_Settings $settings, OpenID_Connect_WP_Client_Wrapper $client_wrapper ) {
	...
	public function make_error_output( $error_code, $error_message ) {
		...
		<div id="login_error"><?php // translators: %1$s is the error code from the IDP. ?>
			<strong><?php printf( esc_html__( 'ERROR (%1$s)', 'openid-connect-wp' ), esc_html( $error_code ) ); ?>: </strong>
	...
	public function make_login_button( $atts = array() ) {
		...
		$atts = shortcode_atts(
			array(
				'button_text' => __( 'Login with OpenID Connect', 'openid-connect-wp' ),
			),
			$atts,
			'openid_connect_generic_login_button'
		);

		$text = apply_filters( 'openid-connect-wp-login-button-text', $atts['button_text'] );
		$text = esc_html( $text );
