...
/**
 * OpenID_Connect_WP_Option_Logger class.
 *
 ...
 * @category Logging
 */
class OpenID_Connect_WP_Option_Logger {
	 ...
	 * @var string
	 */
	const OPTION_NAME = 'openid-connect-wp-logs';
	...
	public function get_logs_table( $logs = array() ) {
		...
		<table id="logger-table" class="wp-list-table widefat fixed striped posts">
			<thead>
				<th class="col-details"><?php esc_html_e( 'Details', 'openid-connect-wp' ); ?></th>
				<th class="col-data"><?php esc_html_e( 'Data', 'openid-connect-wp' ); ?></th>
			</thead>
			<tbody>
			<?php foreach ( $logs as $log ) { ?>
				<tr>
					<td class="col-details">
						<div>
							<label><?php esc_html_e( 'Date', 'openid-connect-wp' ); ?></label>
							<?php print esc_html( ! empty( $log['time'] ) ? gmdate( 'Y-m-d H:i:s', $log['time'] ) : '' ); ?>
						</div>
						<div>
							<label><?php esc_html_e( 'Type', 'openid-connect-wp' ); ?></label>
							<?php print esc_html( ! empty( $log['type'] ) ? $log['type'] : '' ); ?>
						</div>
						<div>
							<label><?php esc_html_e( 'User', 'openid-connect-wp' ); ?>: </label>
							<?php print esc_html( ( get_userdata( $log['user_ID'] ) ) ? get_userdata( $log['user_ID'] )->user_login : '0' ); ?>
						</div>
						<div>
							<label><?php esc_html_e( 'URI ', 'openid-connect-wp' ); ?>: </label>
							<?php print esc_url( ! empty( $log['uri'] ) ? $log['uri'] : '' ); ?>
						</div>
						<div>
							<label><?php esc_html_e( 'Response&nbsp;Time&nbsp;(sec)', 'openid-connect-wp' ); ?></label>
